...
def sobel(img):
    """
    使用Sobel算子提取图像边缘
    输入: img (tensor) - 形状为 [B, 1, H, W] 的掩码
    输出: edge (tensor) - 形状相同的边缘掩码
    """
    # 确保输入是单通道
    ...
    # 定义Sobel算子（水平和垂直方向）
    sobel_x = torch.tensor([[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]], dtype=torch.float32, device=img.device).unsqueeze(
        0).unsqueeze(0)
    sobel_y = torch.tensor([[-1, -2, -1], [0, 0, 0], [1, 2, 1]], dtype=torch.float32, device=img.device).unsqueeze(
        0).unsqueeze(0)
    ...
    edge = edge / edge.max()  # 归一化到 [0, 1]
    return edge

class BoundaryLoss(nn.Module):
    def __init__(self):
        super(BoundaryLoss, self).__init__()
        self.dice = DiceLoss()  # 复用已定义的DiceLoss

    def forward(self, pred, gt):
        # 提取目标边缘
        gt_edge = sobel(gt)  # 现在sobe已定义
        # 预测边缘（先对预测值取阈值得到二值掩码，再提取边缘）
        pred_mask = (pred >= 0.5).float()  # 假设0.5为阈值
        pred_edge = sobel(pred_mask)
        # 用Dice损失计算边缘差异
        return self.dice(pred_edge, gt_edge)
...
class CombinedLoss(nn.Module):
    ...
    def forward(self, pred, gt):
        return 0.7 * self.bce_dice(pred, gt) + 0.3 * self.boundary(pred, gt)